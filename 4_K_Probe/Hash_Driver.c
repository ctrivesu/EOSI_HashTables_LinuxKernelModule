...
{   
    ...
    dev_t current_dev_no = inode->i_rdev;
...
ssize_t ht530_drv_driver_write(struct file *file, const char *buf, size_t count, loff_t *ppos)
{
    ...
    ht_object_t ht_temp;   
    dev_t current_dev_no = file->f_inode->i_rdev;
    ...
    printk(KERN_INFO "WRITE FUNCTION ENTERED - %d\n", MINOR(current_dev_no));
    if(MINOR(current_dev_no)==0)
    {
        ...
        {
            printk(KERN_INFO "HT530 - Writing Element - Key: %d %d Data: 0 - Deleting element", node->ht_obj.key, hash_min(node->ht_obj.key, 7));
        ...
        {
            printk(KERN_INFO "HT530 - Writing Element - Key: %d %d Data: %d", node->ht_obj.key, hash_min(node->ht_obj.key,7), node->ht_obj.data);
...
ssize_t ht530_drv_driver_read(struct file *file, char * buf, size_t count, loff_t *ppos)
{
    ...
    if(MINOR(current_dev_no)==0)
    {
        ...
        if (hnode->first == NULL)
        {
            printk(KERN_INFO "HT530 - Reading - Empty Bucket");
            ht_data.data = -1;
        ...
        if(flag == 1)
        {
            printk(KERN_INFO "HT530 - Reading - Data: %d Key: %d\n",ht_data.data, ht_data.key);
            copy_to_user((ht_object_t *)buf, &ht_data, sizeof(ht_object_t));     
            return 1;
        }   
        ...
        {
            printk(KERN_INFO "HT530 - Reading - Not found in Bucket");
            ht_data.data = -1;
            copy_to_user((ht_object_t *)buf, &ht_data, sizeof(ht_object_t));
            return -EINVAL;
        }
